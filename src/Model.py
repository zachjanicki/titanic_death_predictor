...
class Model(object):
	def __init__(self):
		...
		self.passenger_list = []
		self.survivor_count_test_total = 0
	...
	def calculateSurvivalTotalFromTestData(self, database):
		...
		for passenger in all_passengers:
			...
			# P(class | survival)
			p_class_survival_prob = float(p.boarding_class) / float(self.survivals)
			p_class_death_prob = float(p.boarding_class) / float(self.deaths)
			...
			p_sibling_count_survival_prob = float(p.sibling_count) / float(self.survivals)
			p_sibling_count_death_prob = float(p.sibling_count) / float(self.deaths)
			
			# P(parent/child count | survival)
			p_parent_sibling_count_survival_prob = float(p.parent_child_count) / float(self.survivals)
			p_parent_sibling_count_death_prob = float(p.parent_child_count) / float(self.deaths)
			...
			if p_survival > p_death: #yay!
				self.survivor_count_test_total += 1
			else: # :(
				self.deaths_test_total += 1
		print self.survivor_count_test_total, self.deaths_test_total
